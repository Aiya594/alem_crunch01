...
func NotThree() {
	message := "Error: you only can write 3 symbols in one line"
	PrintError(message)
	ap.PutRune('\n')
}

func drawRow(fill []int, wall rune, player rune, award rune) {
	var we int = len(fill)
	// draw blocks
...
func drawMap(arr [][]int, chars string) {
	...
	for i := 0; i < hei; i++ {
		// draw insides (_/X)
		drawRow(arr[i], wall, player, award)
	}
}
